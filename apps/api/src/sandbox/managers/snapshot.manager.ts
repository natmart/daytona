...
export class SnapshotManager implements TrackableJobExecutions, OnApplicationShutdown {
  ...
  async deactivateOldSnapshots() {
    ...
    try {
      const cutoff = `NOW() - INTERVAL '1 second' * COALESCE(org_timeout."timeout", ${DEFAULT_SNAPSHOT_DEACTIVATION_TIMEOUT_SECONDS})`

      const oldSnapshots = await this.snapshotRepository
        .createQueryBuilder('snapshot')
        .leftJoin(
          `(SELECT rq."organizationId" AS "orgId", MIN(rq."snapshot_deactivation_timeout") AS "timeout" FROM region_quota rq GROUP BY rq."organizationId")`,
          'org_timeout',
          `org_timeout."orgId" = snapshot."organizationId"`,
        )
